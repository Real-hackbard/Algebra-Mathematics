...
uses
  ...
  ExtCtrls, StdCtrls, CheckLst, Grids, RXSpin, Buttons, Mask, XPMan;
...
procedure TForm1.x1C(Sender: TObject);
begin
  d24c(Sender);
end;

procedure TForm1.S46C(Sender: TObject);
begin
  ufaktor:=ufaktor/1.1;
  pb13p(sender);
end;

procedure TForm1.S47C(Sender: TObject);
begin
  ufaktor:=1;
  pb13p(sender);
end;

procedure TForm1.S48C(Sender: TObject);
begin
  ufaktor:=ufaktor*1.1;
  pb13p(sender);
end;

procedure TForm1.P46Resize(Sender: TObject);
begin
...
procedure TForm1.PB13P(Sender: TObject);
var bitmap:tbitmap;
begin
...
procedure TForm1.PB13Paintwmf(can:tcanvas);
...
begin
  ...
  if rb7.checked then begin //figure 2
    kreiskonform(0,0,1);
  ...
  if rb8.checked then begin //figure 3
...
procedure TForm1.konkoord(can:tcanvas;f:double);
...
procedure TForm1.PB12P(Sender: TObject);
var bitmap:tbitmap;
begin
  bitmap:=tbitmap.create;
  bitmap.pixelformat:=pf32bit;
...
procedure TForm1.PB12Paintwmf(can:tcanvas);
...
begin
  ...
  if rb7.checked then begin // figure
...
procedure TForm1.D24C(Sender: TObject);
begin
  pb12p(sender);
  pb13p(sender);
end;

procedure TForm1.PB12MM(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var b,h:integer;
begin
  b:=pb12.width div 2;
...
procedure TForm1.PB12MD(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var b,h:integer;
begin
  b:=pb12.width div 2;
...
procedure TForm1.PB12MU(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var b,h:integer;
begin
  b:=pb12.width div 2;
...
procedure TForm1.D25C(Sender: TObject);
begin
  timer5.enabled:=not timer5.enabled;
  if timer5.enabled then d25.caption:='Cancel'
