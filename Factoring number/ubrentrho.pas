...
uses Forms, Math, SysUtils, Windows, Unit1;
        ...
        while(FGIntCompareAbs(factorGGT, one) = eq)do begin
          ...
          repeat
            ...
            if(d and 7 = 0)then begin
              ...
              QueryPerformanceCounter (t2x);       // Counter reading 2
              form1.label4.caption:=FormatFloat('0.0 ms',1000*(t2x-t1x)/frequenz);
Form1.StatusBar1.Panels[0].Text := Format('D: %d  time: %.1f sec', [d, (t- time)/1000]);
              if abbruch then begin
                Form1.Memo1.Lines.Add(Format('canceled at D: %d', [d]));
                exit;
              end;
            end;
        ...
        if(FGIntCompareAbs(factorGGT, one) <> eq)then begin
          ...
          if(FGIntCompareAbs(A, one) = eq)then begin
            ...
            FGIntAdd(Z, one, Z);      //Try a different Z!!
            Base10StringToFGInt('1', temp2);
            Base10StringToFGInt('1', factorGGT);
            goto neu;
          end else
            form1.memo1.lines.add('Divider = '+s);
...
//          result:= format('%s%s', [result, s]);
      ...
      Form1.StatusBar1.Panels[0].Text:= Format('D: %d    Z: %s    time: %.2f sec   ready!',
