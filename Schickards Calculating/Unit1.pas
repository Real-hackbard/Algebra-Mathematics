...
procedure TForm1.PB8Paintwmf(can:tcanvas);
...
procedure TForm1.PB8MD(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var bo,ho:integer;
...
procedure TForm1.D15C(Sender: TObject);
begin
  fillchar(schick1,sizeof(schick1),0);
  fillchar(schick2,sizeof(schick2),0);
  fillchar(schick3,sizeof(schick3),0);
...
procedure TForm1.PB8MM(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var bo,ho:integer;
    maus:boolean;
begin
...
procedure TForm1.D16C(Sender: TObject);
var k:string;
    i:integer;
begin
  D15C(Sender);
  ...
  if timer2.enabled then button2.caption:='Cancel'
                    else button2.caption:='Multiplication';
...
//1. slider
  ...
  if (schickzaehler>gr4) then begin
    timer2.enabled:=false;
    for i:=2 to 9 do schieber[i]:=false;
    button2.caption:='Multiplication';
  end;
